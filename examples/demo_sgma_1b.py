...
_config_path = os.path.join(os.path.dirname(__file__), ".data_dir_b")
DATA_DIR = open(_config_path).read().strip() if os.path.exists(_config_path) else "."
FILEPATH = os.path.join(DATA_DIR, "IEEE_NE_39_Bus16_fault.csv")

def get_signal(fname):

    Vdata = pd.read_csv(fname).to_numpy()
    time, Vdata = Vdata[:,0], Vdata[:,1:]

    nbus = Vdata.shape[1] // 2
    V = (Vdata[:, :nbus] * np.exp(1j * Vdata[:, nbus:] / 180 * np.pi)).T

    signal = V#np.cumsum(np.diff(V, axis=1), axis=1)
    time = time#[:-1]
...
from sgwt import LENGTH_NEISO as L

# Signals: Real or Complex Matrix (Rows: Buses, Cols: Time)
V, t = get_signal(FILEPATH)
...
temporal_freqs = np.linspace(0.01, 3, 200)
...
plt.savefig(os.path.join(static_images_dir, 'demo_sgma_1b.png'), dpi=400, bbox_inches='tight')
plt.show()
